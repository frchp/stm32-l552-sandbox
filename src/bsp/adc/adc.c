...
static void Adc_PRV_InitSamplingTimer(void)
{
  uint32_t loc_u32TimerClockFreq = 0;             /* Timer clock frequency */
  uint32_t loc_u32TimerPrescaler = 0;                   /* Time base prescaler to have timebase aligned on minimum frequency possible */
  uint32_t loc_u32TimerReload = 0;                      /* Timer reload value in function of timer prescaler to achieve time base period */
  ...
  {
    loc_u32TimerClockFreq = (__LL_RCC_CALC_PCLK1_FREQ(SystemCoreClock, LL_RCC_GetAPB1Prescaler()) * 2);
  }
